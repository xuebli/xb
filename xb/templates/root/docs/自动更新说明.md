...
并默认附带 `--sudoers`（Ubuntu 免密安装 deb 用；Windows 会跳过密码输入，
运行时 sudo 相关能力自动 no-op）。
...
3. `sudo_password` 是唯一例外：**无论何时强制留空**。它是你本机的管理员密码，
   混淆也不允许随包分发，客户端用配置界面设置自己的密码；
...
`datas/logs/update_install.log`，或手动 `sudo dpkg -i` 重试。
...
块版本语义澄清、example 深合并保留占位键、sudo_password 强制留空、
